...
async function main() {
  ...
  // Add a event on the volume
  document.querySelector(".range input").addEventListener("change", (e) => {
    currentSong.volume = parseInt(e.target.value) / 100;
  });

  // Attach event listener to each playlist card
  Array.from(document.getElementsByClassName("card")).forEach(card => {
    card.addEventListener("click", async () => {
      // Get the folder from the dataset
      const folder = card.dataset.folder;
      // Fetch songs from the playlist folder
      await getSongs(`songs/${folder}`);
      // Clear existing songs in the library
      songUL.innerHTML = "";
  ...
  // Event listener to play next song or restart playlist
